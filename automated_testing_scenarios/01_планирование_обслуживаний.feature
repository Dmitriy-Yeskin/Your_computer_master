...
Структура сценария: Я проверяю корректность создания нового документа "Обслуживание клиента" от имени менеджера
	...
	*И реквизит «Ответственный» автоматически заполнился текущим пользователем
		И у элемента с именем 'Ответственный' я жду значения "Менеджер 1" в течение 20 секунд
...
| "Тестовый Контрагент 1" | "Тестовый абонентский договор 1" | "Специалист 1"       | '2000'                      | '500'                    |
| "Тестовый Контрагент 2" | "Тестовый абонентский договор 2" | "Специалист 1"       | '1000'                      | '1000'                   |
| "Тестовый Контрагент 3" | "Тестовый абонентский договор 3" | "Специалист 1"       | '3000'                      | '250'                  	 |
